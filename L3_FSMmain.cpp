...
// ID 추출 함수
int getAssignedID(int cnt) {
    int assignIDparticipant = 123; // 임의의 값
    int assignIDexaminer = 456;
    int assignedID = -1; // 할당된 ID를 저장할 변수, 초기값 -1

    if (cnt == 1) { //참가자일 때
        assignIDparticipant = L3_getPARREQ_result1(uint8_t * msg);
        assignID = assignIDparticipant;
        std::cout << "Assigned ID for participant: " << assignID << std::endl;
    }

    else if (cnt == 2) { //출제자일 때
        assignIDexaminer = L3_getPARREQ_result1(uint8_t * msg);
        if (assignIDexaminer== assignIDparticipant){
            assignIDexaminer++;
        }
        assignedID = assignIDexaminer;
        std::cout << "Assigned ID for examiner: " << assignID << std::endl;
    }
...
void L3_initFSM(uint8_t destId)
{
    ...
    L3_startTimer(20); //initial timer for 심판찾기
